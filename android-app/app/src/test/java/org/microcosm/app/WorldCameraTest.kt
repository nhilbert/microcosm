...
class WorldCameraTest {
    ...
    fun photographTheZoomLadder() {
        ...
        for ((name, cam) in shots) {
            ...
            val painted = px.count { it != Renderer.ABYSS }
            val f = File(out, "world@$name.png")
            FileOutputStream(f).use { bmp.compress(Bitmap.CompressFormat.PNG, 100, it) }
            println("WORLD CAMERA: $name z=${"%.2f".format(cam.z)} -> ${f.absolutePath}")
            assertTrue("$name painted nothing over the abyss", painted > 1000)
        }
    }
}
