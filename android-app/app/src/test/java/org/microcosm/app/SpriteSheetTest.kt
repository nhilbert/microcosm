...
class SpriteSheetTest {
    ...
    fun photographTheBucketGridAndHoldTheDials() {
        ...
        // every bucket paints a real body
        for (row in rows) for ((i, bmp) in row.bmps.withIndex()) {
            assertTrue("${row.name} bucket $i is blank (${lit(bmp)} lit px)", lit(bmp) > 200)
        }
        ...
        for (row in rows) {
            if (row.mN > 1) {
                ...
                assertTrue("${row.name}: morph dial invisible at the rails ($d px differ)", d > 40)
            }
            if (row.tN > 1) {
                val mid = row.mN / 2
                val d = diff(row.bmps[mid], row.bmps[(row.tN - 1) * row.mN + mid])
                assertTrue("${row.name}: tint dial invisible at the rails ($d px differ)", d > 40)
            }
        }
        ...
        val out = File(dir, "sprites.png")
        FileOutputStream(out).use { sheet.compress(Bitmap.CompressFormat.PNG, 100, it) }
        println("SPRITE SHEET: ${rows.size} species grids -> ${out.absolutePath}")
    }
