...
**The sandbox has five openings** (Phase 9, 2026-09-03), not one: `pond` (the
certified world), `still` (one sun, full mineral, nobody home), `twosuns`,
`refuge` (the measured fine-mesh pen) and `shallows` (lean water, M0 1.7). The
table is data in `rust/microcosm-core/src/starts.rs`; the front door's sandbox
row opens the chooser, and the reset button re-founds whichever water you are
in. All five are calibrated over 8 seeds x 18,000 ticks. A sixth — a hot spring
— was measured and deferred with a re-entry condition: any thermal structure
turns thermotaxis on for two species at once and flips the world into a
plankton-dominated regime (12 of 36 sweep runs collapsed). Record:
docs/phase9-starts.md.
