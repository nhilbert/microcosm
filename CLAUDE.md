...
The full running record — 83 entries, phases 1 through 8 — is in
...
render layer and still evolves. Verified on this tree 2026-09-02: `conform`,
`conform:core` (native == wasm), `port:check` (six surfaces), `selfcheck` and
`npm test` all pass, with no stale-hash NOTE. `port:check` is expected to retire
at the first declared ecology change in Rust; that has not happened yet.
...
against `src/ui-render.js` by `harness/fingerprint-frame.js`.
...
thermotaxis, walls with four independent properties. Levers: sun drag and
intensity, mineral pour, feed/kill, species seeding, walls, the Evolution panel
— all logged, undoable, impact-carded. Six Data pages including Traits.

**Phase 8 is where the work is.** Six levels ship (L1–L6); the ladder design
through L12 is in `docs/phase8-ladder-design.md`. Next in owner order: **L9 The
Sorting** (first evolution level), then **L12 Outrun the Sun**, then L7/L11/L8/L10.
All player text obeys `docs/phase8-language-style.md`, enforced by
`harness/prose.js` and `harness/prose-app.js` in `npm test` and CI.

**Open, owner-facing.** The U.2 app UX awaits the owner's next verdict round
(round 3 shipped; a sixth note arrived truncated). Queued behind the ladder: the
post-phase Yoshida study, third loci where a question warrants them, a split
detector once a split is observed. Mycora/Necro re-entry conditions unchanged.
