...
Read plainly: the corridor's rails are safe and the optimiser cannot find the hole, but the interior is not uniformly safe — one keener-grazer/tougher-prey/voracious-decomposer combination starves the decomposer guild on one seed early. By the corridor policy an in-corridor collapse is a documented finding; by the certification standard the scaling note set, the word "certified" is withheld and the exit code says so. **Owner decision (2026-08-29): accepted as an in-corridor finding.** Evolution never reaches the region and the fuzz is the operative guarantee; Bacillus's corridor stays [0,1]. The four-locus world ships as measured.
