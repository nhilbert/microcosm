...
**Re-acceptance.** `tune2` 8/8 ecosystem. Yoshida (corrected estimator): period 3340 → 6555 ± 4299 ticks (n = 7 → 4 seeds with a dominant cycle) — the strongest lengthening measured in any world; phase 0.28 → 0.13; Drifta CV 0.34 → 0.29. Seed 22 captured as the gate's reproduction baseline (5300 → 6080, phase 0.05 → 0.05, g end 0.41). `gate5` **ALL CRITERIA PASS** on the four-locus world (evolution narrated 8/8, control silent 8/8, seed-22 reproduction bit-exact). Corridor (rails + fuzz + 16 interior samples; the 16-corner enumeration was started and stopped as redundant once the sampled certification was running): below.
