...
//
// The gate runs on the REFERENCE world (P.mutation=false, the silent genome): that is
// the world the collapse was certified on, and it reproduces the Phase 4 record to the
// second (strain lead 712 s, control flags 2/78). The evolving world is run afterwards
// for information only — under heredity the grazer's fate changes, which is a Phase 5
// finding, not a Phase 4 failure.
...
function runWorld(seed, decomposers, maxT){
  ...
  for (let t = 0; t < maxT; t++){
    ...
    if (t % 200 === 0 && t > 1200){
      ...
      const cs = ind && ind.strain[GR];
      if (cs) strainLog.push({ t, level: cs.level });
    }
    if (cilioExtinctT < 0){
      let c = 0; for (let i=0;i<W.n;i++) if (W.alive[i] && W.sp[i]===GR) c++;
...
console.log("=== REFERENCE world (P.mutation=false) ===");
...
console.log("\n=== gate criteria ===");
const ext = k6.events.find(e => e.type === "extinct" && e.sp === GR);
...
const falseExt = ok.events.some(e => e.type === "extinct" && SPECIES.CORE.includes(e.sp));
...
// ---- information only: the same experiment on the EVOLVING world (Phase 5) ----
P.mutation = true;
console.log("\n=== EVOLVING world (P.mutation=true), K6 seed 11 — information, not a criterion ===");
const k6e = runWorld(11, false, 18000);
for (const e of k6e.events) console.log(`  t=${e.tick}  ${e.text}`);
const exte = k6e.events.find(e => e.type === "extinct" && e.sp === GR);
console.log(`  grazer extinction: ${exte ? "t="+exte.tick : "none within budget"} (reference: t=${extT})`);
P.mutation = false;
