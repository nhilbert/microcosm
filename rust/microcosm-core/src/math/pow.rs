...
pub fn pow(x: f64, y: f64) -> f64 {
    ...
    // V8's 2016 modification to fdlibm, and the whole reason `pow` needed patching. fdlibm and
    // MUSL compute `(z*t1)/(t1-2) - (w + z*w)`; V8 divides by the ENTIRE denominator instead.
    // Undocumented in the V8 source, worth 4.4% of samples, and the difference between a port that
    // reproduces the certified world and one that quietly becomes a different world.
    // (V8 12.4 src/base/ieee754.cc: `r = base::Divide(z * t1, (t1 - two) - (w + z * w));`)
